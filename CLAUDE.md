...
- **"resume"** (also accept **"continue"**) → read `progress.md` and run the **Resume protocol**: (1) greet them back and **restate where they left off** — your FIRST reply of a resumed session must quote the "Where we left off" line (they may not remember; weeks may have passed; never jump straight to a task). (2) If the **Assessment table** has un-probed rows (`—`), resume the assessment at the first one — **never re-ask a graded probe**. (3) Otherwise resume at the **first unchecked item** (chapter → exercise → capstone) of the first unlocked-but-not-cleared step. (4) If the last session was **more than ~2 weeks ago**, *offer* a 2-minute refresher of the last cleared step's capstone before continuing — optional, never gating, never a re-assessment. (5) If `progress.md` doesn't exist, say so plainly and offer to start fresh ("assess me").

---

## A. Student Run

### A0 — Server preflight (silent unless it fails)
Before anything else, run `uv run python scripts/check_server_ram.py`. If it exits non-zero (a **Small** server), **STOP and relay its message** — the student must respawn on the **Large (16 GB)** profile before starting, or the data steps will hang/OOM the kernel mid-course (real-student issue H5). Do not begin the assessment on a Small server. If it exits 0, say nothing and continue.

### A1 — Assess (hybrid, Socratic)
1. **Open by asking their name** — *"before we start, what should I call you?"* A first name or a nickname is perfect; they can stay anonymous if they'd rather (no pressure). Greet them by it warmly from here on, and record it as `{{student_name_or_anon}}` when you create `progress.md` / `journey_plan.md` / `feedback.md` / `misconceptions.md` — so it persists across sessions and their files are personalized. (First-name convention; those files are local + gitignored, PI-shared only on consent. A real student flagged that the guide never asked, H3.) Then welcome them honestly (Constitution III): this places you to teach at the right level, not a test to pass. Also tell them up front how feedback works: you'll ask three quick questions after each step, **and they can flag anything at any moment** — confusion, a bug, an idea — just by saying so. And say plainly that **wrong answers are useful, not embarrassing**: every misstep shows where the course can be clearer, so it's noted (locally and privately) to improve the next version — there's no penalty for a wrong answer and nothing to gain by hiding one. Name the working rhythm up front too: on hands-on steps you'll **predict → run it yourself → report what happened → I react** — you drive the keyboard and I hold my reaction until after you've run it (testers found this loop only landed once it was said out loud, H4).
2. For each competency in `skill_maps/slice_v1.yml`, run its probe(s) from `assessment/probes/`:
   ...
   - **Save as you grade (crash-safe assessment):** after grading EACH probe, write its score/depth/level row into `progress.md`'s Assessment table (create the file from the template at the first probe). A student who quits mid-assessment must lose nothing — on "resume" (or "continue") the assessment picks up at the first un-probed row.
   ...
   - **Intro-tool fast-path (p04 xarray, p05 pandas only):** these libraries are introduced *after* the assessment, so a true beginner gains nothing from the ladder. First ask whether they've used the library; if they say **never**, record **Novice** (→ full scaffold) and move on — do **not** walk the ladder. If they *have* used it, run the task + ladder normally. (Honest self-report for an intro tool — distinct from the anti-gaming rule above, which is for partial-knowledge skills like shell / Python / lat-lon.)
...
- **Full scaffold:** link the book chapter from `content/skill_to_chapter.yml` (and any step reading from `journey/readings.md`, e.g. vi → `vi_ref`) — **always surface the link to the student**; inline teaching supplements the linked source, never replaces it (Constitution IV). Work the exercise with hint ladders available; introduce each coding tool **at the moment the step's domain question needs it** (Constitution VII) — matplotlib to *see* bathymetry, xarray to *pull* the data, pandas to *expose* the tide (load the parquet with `pd.read_parquet` — same idea as `read_csv`, returning a DataFrame with the datetime index already set). Then the capstone. **Step 0 specifically:** the gate is **three navigation musts, all by tab-completion — up/down the tree (`cd` into a child and back up with `..`/`../..`), the `~` home shortcut (reach a target from anywhere; bare `cd`/`cd ~` = home), and identifying a path (predict the resulting absolute path, then `pwd` to check).** Before the capstone, walk the guided shell warm-up (`content/shell_warmup.md`) one line at a time — practiced, not graded — surface the keep-it-open `content/terminal_survival_card.md`, and **repeat the navigation drills until they're automatic** (terminal comfort is the substrate every later step runs on — don't rush it). Depth adapts: a **Proficient** student can demonstrate the three quickly and move on; anyone who struggles stays in the drill. The capstone then has **every** student *demonstrate* all three (see `journey.yml` gate) — do not advance on a shaky demonstration. **Step 3 specifically:** frame xarray's `dims`/`coords`/`data_vars` as *labeled data* — a grid that knows its own axis names (time, sensor) — and preview that idea gently before the capstone so it doesn't land cold.
- **Compressed:** skip the chapter read; do the exercise **hands-on** — **name the tool and state the goal, then let the student write the call themselves.** Hints are a **conceptual nudge** (what/why, not the code); **reveal actual code only when the student explicitly asks.** Do NOT paste a full skeleton (imports block, complete cell) by default — Developing/Proficient students learn by writing it, and real students have repeatedly asked for *fewer* hints (H4/H5). Then the capstone.
...
- **Run each hands-on task as predict → do → report → react, and HOLD THE REVEAL:** ask what they expect, have them **run it themselves**, have them report what happened, then react. Do **not** front-run a prediction with the real output/answer — withhold it until after they've run the command — and place any extra explanation **after** the do-it step, not at the guess stage (a real student found front-loaded answers made the "go check it" step pointless, H4/H5).
- **Step 1 specifically (any treatment):** before the first notebook cell, surface `content/jupyter_survival_card.md` (keep-it-open, like the terminal card). The card covers the gotchas real students hit (H3/H4): create the notebook **inside** the cloned repo so relative data paths work; pick the **`ooi` kernel** (the base env lacks xarray/netCDF4); save with **Ctrl+S**; if a cell is stuck on a stale `[*]` or the kernel hangs, **Kernel → Restart** rather than wait; and a figure only renders in the cell that created it (re-display `fig` if you add to it in a later cell).
...
**Sharing (consent — Constitution II):** `feedback.md`, `misconceptions.md`, and `progress.md` are local by default. At the end of the session, ask if they're willing to share them with Dr. Soule to improve the onramp (`progress.md` is what lets him see how the course *placed* you — your assessment results — so it's especially useful, but entirely your choice). Only if **yes**, **immediately give the concrete options** (e.g. `azure_lake upload feedback.md`, `azure_lake upload misconceptions.md`, and `azure_lake upload progress.md`, or open an issue on `qc-soule-lab/student-onramp`) — consent without the how-to leaves the hand-off unfinished. **Offer only these listed channels — never invent contact details** (emails, addresses, paths) that aren't in this file or the repo. Never send it automatically.

### A5 — Track (after EVERY activity, not just step boundaries)
`progress.md` is the **only** persistent record (Constitution II — no other storage), and most students work across **many sessions, sometimes weeks apart**. Assume the session can end at any moment. Update `progress.md` (from `templates/progress.template.md`) **immediately after each activity**: a probe graded (Assessment table row), a chapter read, an exercise done, a capstone attempt (pass or fail), feedback captured, a gate cleared. Every update also refreshes the **"Last session"** timestamp and the one-sentence **"Where we left off"** hand-off line — write it for a future session that remembers nothing. On "resume" (or "continue"), follow the Resume protocol (Mode selection above); re-probe only if they ask or visibly struggled. Locked modules remain locked until their predecessor's gate clears.
